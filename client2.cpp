...
void init_TCP(){
    ...
    // client id?????
    cout << "Client2 is up and running" << endl;
}
...
void send_query (string country, string id){
    ...
    // client id ??? 
    cout << "Client2 has sent User " << id  << " and " << country << " to Main Server using TCP" << endl;
}
...
void receive_result(){
    ...
    if (command == "NO_COUNTRY"){        // NO_COUNTRY
        ...
        // client id ????????
		cout << "Client2 has results from Main Server :" << all_recommand_users << "is/are possible friend(s) of User " << id << " in " << country << endl;
    }
